...
**Датчики температуры и влажности** (17.09.2026): два eWeLink CK-TLSR8656 (`kitchen`, `bedroom` — имена
даны наугад, пользователь ещё не сказал, какой где; переименовать через `bridge/request/device/rename`).
...
рисует табличку прямо на скетче — на стене между левым бра и картиной (там единственное пустое место:
бра кончается на x=472, картина начинается с x=560). Текст внутри `.ink` надо возвращать к заливке
