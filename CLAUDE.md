...
панель: тумблеры, яркость, тепло (кельвины → mired), сцены и эффекты — через `serve.py`, а не через CLI
`lamp` из скилла. Сцены только белые: «Уютно» пользователя (43 %, 2300 K) плюс рецепты Philips Hue (Rest, Read, Bright,
