...
(сейчас там остаток на 5 тысяч за восемь лет). Крупным считается чек от `big` = 100 000 ₽.
...
- **Регулярные списания** — не меньше четырёх штук с медианным интервалом 24–38 дней **и** растянутые
  минимум на `RECUR_SPAN` = 100 дней (без этого три похода за одеждой подряд читались как подписка).
...
**График — один цвет.** Пара «терракота + шалфей» из `theme.css` проваливает проверку как категориальная
(ΔE 14,9 при пороге 15 — неразличимы даже при полном зрении), а новых цветов на странице заводить нельзя.
Поэтому столбики одного тона, «крупное» отличается штриховкой, своим масштабом и прямой подписью, а
категории внутри месяца — список с полосками, где различает не цвет, а подпись.
...
(525 уникальных чеков против 508 и 424 по отдельности). Оттуда же приезжает Сбермаркет, которого в
