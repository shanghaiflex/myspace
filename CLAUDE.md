...
Плашка рисует такое событие пунктиром (`busy: false` → класс `free-ev`), окно она не режет — в том числе врачи
и няня; если заметка позовёт гулять в час приёма, вот почему. Плавание Полины по-прежнему не показывается:
четыре часа чужой тренировки — не дело дня.
