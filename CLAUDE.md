...
Календари: «Мои события» (повторяющиеся правила быта), «Семья» (общий, живой), «Тренировки» (плавание Полины),
«Не забыть» (todo, пропускается). События каждого помечаются `X-BOW-CALENDAR` при сборке — `CATEGORIES` Яндекс
заполняет только в одном календаре. Календари из `SCHEDULE_FREE_CALENDARS` (по умолчанию «Тренировки») в дне
показываются, но свободное окно не съедают: четыре часа чужого плавания каждый понедельник и пятницу не
оставляли в утре места. Так же ведут себя события, помеченные в Яндексе «свободен» (`TRANSP:TRANSPARENT`) —
например «Французский Сережи». Свободное окно — дыра не меньше `FREE_MIN` (45 мин) между `DAY_START` и `DAY_END`.
...
Состояние на 14.09.2026: закрыты «Психолог чт 09:00» и «ЛФК ср 18:30» (вместо неё заведена ср 18:00).
Подтверждены как действующие: «Работа пн–пт 10:00–18:00», «Французский Сережи вт/чт 19:00». Под вопросом
остались «Теннис сб 16:00» (по часам теннис по вторникам около 09:30) и «Тренировка в зале» ср 09:00 / вс 10:00
(силовых в данных часов за 12 недель нет).
