...
Живой огонь (камин/свеча/факел) со страницы убран 14.09.2026 по просьбе пользователя; `effect.js` остался только для CLI. Сцены только белые: «Уютно» пользователя (43 %, 2300 K) плюс рецепты Philips Hue (Rest, Read, Bright,
Concentrate, Nightlight) с температурами Hue и яркостью, умноженной на 0,76 — цифры Hue один к одному на Tuya
светят ярче, чем задумано (Dimmed 30 % выглядел как свет для чтения); «Приглушённо» — это «Уютно» на 18 %; цветные пресеты в духе WiZ (янтарь,
