...
`photos` (фото блюда с Wikimedia Commons). Всё детерминированное; модель пишет только текст
заметки и идеи блюд.
...
профиль покупок (`pantry.py profile`: доли категорий + регулярные товары за все годы) и возвращает
JSON из 4 блюд. Ограничения жёсткие и намеренные: ≤6 ингредиентов, ≤4 шага, ≤20 минут — по чекам
видно, что человек покупает много готового, длинные рецепты мимо.

Фото — Wikimedia Commons (ключа не нужно, покрывает русские блюда, не режет лимитами как Openverse).
`photos.py` фильтрует диаграммы и упаковку, требует ширину ≥900 и **отбрасывает снимки, в названии
которых есть форма блюда, не упомянутая в запросе** — иначе «creamy mushrooms with bacon» возвращает
карбонару. Запрос укорачивается по словам, пока Commons что-то не найдёт; ответы кэшируются
(`data/commons_cache`), между запросами пауза 3 с. Фото подписано как иллюстрация, а не снимок
этого блюда, — потому что это правда, и потому что CC требует атрибуции.
