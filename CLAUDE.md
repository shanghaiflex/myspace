...
«Французский вт/чт 19:00» (15.09 переименован из «Французский Сережи» — это своё занятие, а не чужое;
`rename` меняет SUMMARY, сохраняя UID, поэтому серия с её историей и исключениями остаётся одним событием),
«ЛФК ср 18:00».
...
### Главная — полоса выпуска, а не плитки

Порядок карточек прежний (см. ниже), но вид другой (18.09.2026, по просьбе пользователя: «много блоков,
какой-то хаос»). Десять белых плашек с рамками, тенями и чёрными кнопками, лежащих поверх фотографии на
всю страницу, — это и был хаос. Теперь каждая заметка начинается волосяной линейкой сверху и стоит прямо
на бумаге: ни фона, ни рамки, ни тени. Фотография осталась, но стала обложкой — полоса 430 px вверху,
растворяющаяся в бумаге, а не подложка под всем текстом. Кнопки («Слушать дальше», «Играть», «Открыть»)
стали подписями киноварью прописными: восьми чёрных плашек на экране больше нет. Кнопки не прижимаются
к низу карточки (`margin-top: auto` убран) и карточка не растягивается на высоту строки
(`align-self: start`) — из-за этого соседние заметки раньше расходились рваными столбцами.
Врезка «Чем занять окно» — три колонки, разделённые линейками; часы погоды — тоже через линейки.
Верстка живёт в самой странице, как и у всех остальных: в `theme.css` только палитра и типографика.
...
«Дела на сегодня», «Чем занять окно», погода, «Well-being», лекция, «Почитать», два микса (случайный + совет
Claude), «Читаю», «Французский» (добавлен 15.09.2026 в конец — чтобы не двигать уже заданный порядок). На
телефоне сетка складывается в одну колонку (`@media (max-width: 900px)`), то есть порядок в приложении BoW —
тот же самый.
