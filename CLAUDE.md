...
подряд и никогда по кругу — порядок и есть защита от дальтонизма. Набор прогнан валидатором (OKLab,
симуляция Machado 2009) против светлой и тёмной поверхности и проходит все шесть проверок: светлота в
полосе, цветность выше пола, худшая соседняя пара ΔE 12,4 при дальтонизме и 17,3 при полном зрении,
контраст к фону ≥ 3:1. Один и тот же набор работает в обеих темах, поэтому в тёмной он не переопределяется.
...
нейтральный `--c0` — восьми хватает с запасом, по кругу цвета не идут никогда. Между сегментами 2 px фона,
