...
HOST="${MINI_HOST:-mini}"
MSG="${1:-Update site}"
DATA="movies.json mixes.json lectures.json books.json mix_recs.json taste_recs.json"
...
if [ -z "$NOMINI" ]; then
  rsync -az --delete --exclude .git --exclude audio --exclude .env --exclude .session_secret --exclude logs --exclude "health.db*" --exclude backups --exclude pantry.json --exclude dishes --exclude commons_cache --exclude "scripts/pantrylib/data/receipts.json" --exclude __pycache__ --exclude .DS_Store ./ "$HOST:movies/"
