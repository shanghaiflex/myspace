...
if [ -z "$NOMINI" ]; then
  rsync -az --delete --exclude .git --exclude audio --exclude .env --exclude .session_secret --exclude logs --exclude "health.db*" --exclude backups --exclude pantry.json --exclude dishes --exclude reads/img --exclude french/img --exclude data/calendar.ics --exclude data/calendar-deleted --exclude data/lamp-phase.json --exclude commons_cache --exclude "scripts/pantrylib/data/receipts.json" --exclude __pycache__ --exclude .DS_Store ./ "$HOST:movies/"
