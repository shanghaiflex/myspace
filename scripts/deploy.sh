...
if [ -z "$NOMINI" ]; then
  rsync -az --delete --exclude .git --exclude audio --exclude .env --exclude .session_secret --exclude logs --exclude "health.db*" --exclude backups --exclude pantry.json --exclude agent_tasks.json --exclude "agent/reports" --exclude dishes --exclude commons_cache --exclude "scripts/pantrylib/data/receipts.json" --exclude __pycache__ --exclude .DS_Store ./ "$HOST:movies/"
